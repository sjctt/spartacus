...
<div>
  <div class="d-flex flex-row chat-heading">
        <div class="d-flex">
          <img :alt="otherUser.title" :src="otherUser.img" class="img-thumbnail border-0 rounded-circle ml-0 mr-4 list-thumbnail align-self-center small"/>
        </div>
        ...
        <div  v-for="(message,index) in messages" :key="`message${index}`">
          <b-card no-body
           ...
           :class="{'d-inline-block mb-3' : true, 'float-left':message.sender==otherUser.id, 'float-right':message.sender==currentUser.id}">
            ...
            <div class="position-absolute  pt-1 pr-2 r-0"><span class="text-extra-small text-muted">{{message.time}}</span></div>

            <b-card-body>
              <div class="d-flex flex-row pb-1" v-if="message.sender==currentUser.id">
                <img :alt="currentUser.title" :src="currentUser.img" class="img-thumbnail border-0 rounded-circle mr-3 list-thumbnail align-self-center xsmall"/>
               ...
               <div class="d-flex flex-row pb-1" v-else>
                 <img :alt="otherUser.title" :src="otherUser.img" class="img-thumbnail border-0 rounded-circle mr-3 list-thumbnail align-self-center xsmall"/>
                <div class=" d-flex flex-grow-1 min-width-zero">
                  <div class="m-2 pl-0 align-self-center d-flex flex-column flex-lg-row justify-content-between min-width-zero">
                    <div class="min-width-zero">
